...
with 
member [DIM DATE].[Day Light Saving].[Day Light Saving] as
	iif(
		[DIM DATE].[Date].currentmember >= '2023-03-12'
		and [DIM DATE].[Date].currentmember <= '2023-11-05',
		'True', 'False'
	)
member [Measures].[Mean AQI] as
	avg(
...
select 
	...
	nonemptycrossjoin(
		...
		[DIM CATEGORY].[Daily Aqi Color].[Daily Aqi Color].members
	) on rows
from [US AQI DDS]
where [DIM DATE].[Day Light Saving].&[False];
...
select 
	...
	nonemptycrossjoin(
		[DIM COUNTY].[State Name].[State Name].members,
		[DIM CATEGORY].[Daily Aqi Color].members
	) on rows
from [US AQI DDS]
...
select 
	[DIM CATEGORY].[Daily Aqi Color].members on columns,
